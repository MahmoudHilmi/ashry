...
const CATEGORIES = ["الجميع", ...new Set(MOCK_PRODUCTS.map(p => p.category))];
const SORT_OPTIONS = [
...
async function fetchProducts() {
  ...
  return MOCK_PRODUCTS.map((p, i) => ({ ...p, id: String(i + 1) }));
}
...
// ─── GRID PAGE ────────────────────────────────────────────────────────────────
function GridPage({ products, loading, error, onNavigate }) {
      ...
      >
        ...
        {!loading && !error && displayed.length > 0 && (
          <>
            ...
            {/* KEY FIX: auto-fill grid fills full width on every screen */}
            <div
              style={{
                display: "grid",
                gridTemplateColumns:
